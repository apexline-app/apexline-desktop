...
- **`@apexline-app/http-toolkit-js@0.1.2` migration** — replaced ad-hoc envelope fetcher in `src/shared/api/auth-fetch.ts` with the toolkit's `createHttpClient`:
  ...
  - `features/auth/main/handler.ts` calls `initApiClient` in `registerAuthHandlers` with token/refresh callbacks bound to local auth state. `persistSession` (`/me`) and `handleSignUp` (`/api/v1/users`) use `getApiClient().get/post` with Zod schema validation; envelope unwrapping, snake/camel boundary, retries (`502/503/504` + `429 Retry-After`), `X-Request-Id`, `X-AppVersion`, and 401-refresh are owned by the toolkit.
